...
class CORSOriginHeadersTest(BaseWebTest, unittest.TestCase):
    ...
    def test_present_on_unknown_url(self):
        ...
        self.assertEqual(
            response.headers["Access-Control-Allow-Origin"], "notmyidea.org"
        )

    def test_not_present_on_unknown_url_if_setting_does_not_match(self):
        with mock.patch.dict(
            self.app.app.registry.settings, [("cors_origins", "daybed.io")]
        ):
            response = self.app.get("/unknown", headers=self.headers, status=404)
    ...
    def test_present_on_readonly_update(self):
        with mock.patch(
            "tests.core.testapp.views.MushroomSchema.is_readonly", return_value=True
        ):
    ...
    def test_present_on_unauthorized(self):
        ...
        body = {"data": MINIMALIST_RECORD}
        response = self.app.post_json(
            self.collection_url, body, headers=headers, status=401
        )
        self.assertIn("Access-Control-Allow-Origin", response.headers)
...
class CORSExposeHeadersTest(BaseWebTest, unittest.TestCase):
    def assert_expose_headers(
        self, method, path, allowed_headers, body=None, status=None
    ):
...
class CORSMaxAgeTest(BaseWebTest, unittest.TestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.headers.update(
            {"Origin": "lolnet.org", "Access-Control-Request-Method": "GET"}
        )
