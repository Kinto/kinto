...
class LenientMemoryStorageTest(MemoryStorageTest):
    ...
    def test_update_bytes_raises(self):
        ...
        self.assertIsNotNone(
            self.storage.update(
                object_id=record["id"], record=new_record, **self.storage_kw
            )
        )
...
class PostgreSQLStorageTest(StorageTest, unittest.TestCase):
    ...
    def test_warns_if_configured_pool_size_differs_for_same_backend_type(self):
        ...
        msg = (
            "Reuse existing PostgreSQL connection. Parameters storage_* "
            "will be ignored."
        )
        with mock.patch(
            "kinto.core.storage.postgresql.client." "warnings.warn"
        ) as mocked:
