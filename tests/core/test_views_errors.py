...
class ErrorViewTest(FormattedErrorMixin, BaseWebTest, unittest.TestCase):
    ...
    def test_backoff_headers_is_present_if_less_than_percentage(self):
        with mock.patch.dict(
            ...
            [("backoff", 10), ("backoff_percentage", 50)],
        ):
            with mock.patch(
                "kinto.core.initialization.random.random", return_value=0.4
            ):
                response = self.app.get(
                    self.sample_url, headers=self.headers, status=200
                )
        self.assertIn("Backoff", response.headers)
        self.assertEquals(response.headers["Backoff"], "10")
    ...
    def test_backoff_headers_is_not_present_if_greater_than_percentage(self):
        with mock.patch.dict(
            ...
            with mock.patch(
                "kinto.core.initialization.random.random", return_value=0.6
            ):
                response = self.app.get(
                    self.sample_url, headers=self.headers, status=200
                )
    ...
    def test_backoff_header_is_present_on_error_responses(self):
        with mock.patch.dict(self.app.app.registry.settings, [("backoff", 10)]):
            with mock.patch(
                ...
                response = self.app.get(
                    self.sample_url, headers=self.headers, status=500
                )
        self.assertIn("Backoff", response.headers)
    ...
    def test_500_logs_request_information(self):
        with mock.patch(
            ...
            with mock.patch("kinto.core.views.errors.logger") as mocked_logger:
                self.app.get(
                    self.sample_url + "?q=-42", headers=self.headers, status=500
                )
...
class RedirectViewTest(FormattedErrorMixin, BaseWebTest, unittest.TestCase):
    ...
    def test_querystring_is_preserved_during_redirection(self):
        ...
        self.assertEqual(
            response.location, "http://localhost/v0/home/articles?_since=42"
        )

    def test_redirection_does_not_allow_crlf(self):
        self.app.get(
            "/crlftest%0DSet-Cookie:test%3Dtest%3Bdomain%3D.yelp.com", status=404
        )
...
class TrailingSlashRedirectViewTest(
    FormattedErrorMixin, BaseWebTest, unittest.TestCase
):
