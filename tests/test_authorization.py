...
class ResourceEndpointTest(unittest.TestCase):
    def test_resource_endpoint_return_right_type_for_key(self):
        ...
        self.assertEqual(
            _resource_endpoint("/buckets/bid/collections/cid"), ("collection", False)
        )
        self.assertEqual(_resource_endpoint("/buckets"), ("", False))
        self.assertEqual(_resource_endpoint("/buckets/bid"), ("bucket", False))
        self.assertEqual(
            _resource_endpoint("/buckets/bid/history/xx"), ("history", False)
        )
        self.assertEqual(
    ...
    def test_resource_endpoint_return_right_type_for_children_collection(self):
        ...
        self.assertEqual(
            _resource_endpoint("/buckets/bid/collections"), ("bucket", True)
        )
...
class RelativeObjectUri(unittest.TestCase):
    ...
    def test_related_object_uri_can_construct_parents_set_uris(self):
        ...
        self.assertEqual(
            _relative_object_uri("collection", self.record_uri), self.collection_uri
        )
        self.assertEqual(
            _relative_object_uri("collection", self.records_uri), self.collection_uri
        )

        # Can build bucket_uri from obj_parts
        self.assertEqual(
            _relative_object_uri("bucket", self.record_uri), self.bucket_uri
        )
        ...
        self.assertEqual(
            _relative_object_uri("bucket", self.group_uri), self.bucket_uri
        )
        self.assertEqual(
            _relative_object_uri("bucket", self.groups_uri), self.bucket_uri
        )
    ...
    def test_relative_object_uri_fail_construct_children_set_uris(self):
        ...
        self.assertRaises(
            ValueError, _relative_object_uri, "collection", self.bucket_uri
        )
...
class PermissionInheritanceTest(unittest.TestCase):
    ...
    def test_inherited_permissions_supports_buckets_named_collections(self):
        ...
        self.assertEquals(
            set(_inherited_permissions(uri, "write")), set([(uri, "write")])
        )
