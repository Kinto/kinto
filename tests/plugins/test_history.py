...
class HistoryViewTest(HistoryWebTest):
    ...
    def test_history_supports_creation_via_plural_endpoint(self):
        resp = self.app.post_json(
            "/buckets", {"data": {"id": "posted"}}, headers=self.headers
        )
    ...
    def test_bucket_delete_destroys_its_history_entries(self):
        ...
        stored_in_backend, _ = storage.get_all(
            parent_id="/buckets/test", collection_id="history"
        )
    ...
    def test_delete_all_buckets_destroys_history_entries(self):
        ...
        storage = self.app.app.registry.storage
        stored_in_backend, _ = storage.get_all(
            parent_id="/buckets/1", collection_id="history"
        )
        assert len(stored_in_backend) == 0
    ...
    def test_tracks_record_creation(self):
        ...
        assert entry["action"] == "create"
        assert entry["uri"] == "/buckets/test/collections/{}/records/{}".format(
            cid, rid
        )
    ...
    def test_tracks_record_attributes_update(self):
        resp = self.app.patch_json(
            self.record_uri, {"data": {"foo": "baz"}}, headers=self.headers
        )
        resp = self.app.get(self.history_uri, headers=self.headers)
        entry = resp.json["data"][0]
        assert entry["action"] == "update"
...
class HistoryDeletionTest(HistoryWebTest):
    def setUp(self):
        ...
        body = {"data": {"foo": 42}}
        self.app.put_json(
            "/buckets/bid/collections/cid/records/rid", body, headers=self.headers
        )
    ...
    def test_partial_deletion(self):
        ...
        self.app.delete(
            "/buckets/bid/history?_before={}".format(before), headers=self.headers
        )
...
class FilteringTest(HistoryWebTest):
    def setUp(self):
        ...
        body = {"data": {"foo": 0}}
        self.app.put_json(
            "/buckets/0/collections/1/records/2", body, headers=self.headers
        )
        body = {"data": {"foo": "bar"}}
        self.app.patch_json(
            "/buckets/bid/collections/cid/records/rid", body, headers=self.headers
        ...
        self.app.delete(
            "/buckets/bid/collections/cid/records/rid", headers=self.headers
        )

    def test_filter_by_unknown_field_is_not_allowed(self):
        self.app.get(
            "/buckets/bid/history?movie=bourne", headers=self.headers, status=400
        )
    ...
    def test_filter_by_resource(self):
        resp = self.app.get(
            "/buckets/bid/history?resource_name=bucket", headers=self.headers
        )
        assert len(resp.json["data"]) == 1
    ...
    def test_filter_by_uri(self):
        ...
        resp = self.app.get(
            "/buckets/bid/history?uri={}".format(uri), headers=self.headers
        )
    ...
    def test_allows_diff_between_two_versions_of_a_record(self):
        ...
        resp = self.app.get(
            "/buckets/bid/history{}".format(querystring), headers=self.headers
        )
    ...
    def test_filter_returned_fields(self):
        resp = self.app.get(
            "/buckets/bid/history?_fields=uri,action", headers=self.headers
        )
        assert sorted(resp.json["data"][0].keys()) == [
            "action",
            "id",
            "last_modified",
            "uri",
...
class PermissionsTest(HistoryWebTest):
    ...
    def setUp(self):
        ...
        self.julia_principal = (
            "basicauth:d8bab8d9fe0510fcaf9b5ad5942c027fc"
    ...
    def test_read_permission_can_be_given_to_anybody_via_settings(self):
        with mock.patch.dict(
            ...
            resp = self.app.get(
                "/buckets/test/history", headers=get_user_headers("tartan:pion")
            )
            entries = resp.json["data"]
            assert len(entries) == 6  # everything.
    ...
    def test_bucket_read_allows_whole_history(self):
        ...
        self.app.get(
            "/buckets/author-only/history", headers=self.alice_headers, status=403
        )
    ...
    def test_new_entries_are_not_readable_if_permission_is_removed(self):
        ...
        self.app.patch_json(
            "/buckets/test", {"permissions": {"read": []}}, headers=self.headers
        )
