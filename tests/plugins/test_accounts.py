...
class AccountCreationTest(AccountsWebTest):
    def test_anyone_can_create_an_account(self):
        self.app.post_json(
            "/accounts", {"data": {"id": "alice", "password": "12éé6"}}, status=201
        )

    def test_account_can_be_created_with_put(self):
        self.app.put_json(
            "/accounts/alice", {"data": {"password": "123456"}}, status=201
        )
    ...
    def test_authentication_is_accepted_if_account_exists(self):
        self.app.post_json(
            "/accounts", {"data": {"id": "me", "password": "bouh"}}, status=201
        )
    ...
    def test_authentication_does_not_call_bcrypt_twice(self):
        ...
        with mock.patch(
            "kinto.plugins.accounts.authentication.bcrypt"
        ) as mocked_bcrypt:
            resp = self.app.get("/", headers=get_user_headers("me", "bouh"))
            assert resp.json["user"]["id"] == "account:me"
...
class AccountUpdateTest(AccountsWebTest):
    ...
    def test_authentication_with_old_password_is_denied_after_change(self):
        ...
        self.app.get(
            "/accounts/alice", headers=get_user_headers("alice", "123456"), status=401
        )
...
class AccountViewsTest(AccountsWebTest):
    ...
    def test_cannot_obtain_someone_else_account(self):
        self.app.get(
            "/accounts/bob", headers=get_user_headers("alice", "123456"), status=403
        )

    def test_cannot_obtain_unknown_account(self):
        self.app.get(
            "/accounts/jeanine", headers=get_user_headers("alice", "123456"), status=403
        )
...
class AdminTest(AccountsWebTest):
    ...
    def setUp(self):
        self.app.put_json(
            "/accounts/admin", {"data": {"password": "123456"}}, status=201
        )
    ...
    def test_admin_can_get_other_accounts(self):
        self.app.put_json(
            "/accounts/alice", {"data": {"password": "azerty"}}, status=201
        )

        resp = self.app.get(
            "/accounts/alice", headers=get_user_headers("alice", "azerty")
        )
    ...
    def test_admin_can_delete_other_accounts(self):
        ...
        self.app.get(
            "/accounts/bob", headers=get_user_headers("bob", "987654"), status=401
        )
    ...
    def test_user_created_by_admin_with_put_can_see_her_record(self):
        ...
        resp = self.app.get(
            "/accounts/alice", headers=get_user_headers("alice", "bouh")
        )
...
class CreateUserTest(unittest.TestCase):
    ...
    def test_create_user_in_read_only_displays_an_error(self):
        with mock.patch("kinto.plugins.accounts.scripts.logger") as mocked:
            ...
            mocked.error.assert_called_once_with(
                "Cannot create a user with " "a readonly server."
            )
    ...
    def test_create_user_with_an_invalid_username_and_password_confirmation_recovers(
        self
    ):
        with mock.patch(
            "kinto.plugins.accounts.scripts.input", side_effect=["&zert", "username"]
        ):
            with mock.patch(
    ...
    def test_create_user_with_a_valid_username_and_password_confirmation(self):
        with mock.patch(
            "kinto.plugins.accounts.scripts.input", return_value="username"
        ):
            with mock.patch(
                "kinto.plugins.accounts.scripts.getpass.getpass",
