...
class LoginViewTest(OpenIDWebTest):
    def test_returns_400_if_parameters_are_missing_or_bad(self):
        ...
        self.app.get(
            "/openid/auth0/login", params={"callback": "http://no-scope"}, status=400
        )
        self.app.get(
            "/openid/auth0/login",
            params={"callback": "bad", "scope": "openid"},
            status=400,
        )
    ...
    def test_returns_400_if_email_is_not_in_scope_when_userid_field_is_email(self):
        ...
        cb = "http://ui"
        self.app.get(
            "/openid/auth0/login", params={"callback": cb, "scope": scope}, status=307
        )
        # See config above (email is userid field)
        self.app.get(
            "/openid/google/login", params={"callback": cb, "scope": scope}, status=400
        )
...
class TokenViewTest(OpenIDWebTest):
    ...
    def test_returns_400_if_state_is_invalid(self):
        self.app.get(
            "/openid/auth0/token", params={"code": "abc", "state": "abc"}, status=400
        )
