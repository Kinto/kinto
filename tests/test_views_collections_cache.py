...
class GlobalSettingsTest(BaseWebTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app.put_json(
            "/buckets/blog/collections/cached",
            MINIMALIST_COLLECTION,
            headers=self.headers,
        )
        r = self.app.post_json(
            "/buckets/blog/collections/cached/records",
            MINIMALIST_RECORD,
            headers=self.headers,
        )
...
class SpecificSettingsTest(BaseWebTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        def create_record_in_collection(bucket_id, collection_id):
            ...
            self.app.put_json(
                "/buckets/{}".format(bucket_id), bucket, headers=self.headers
            )
            collection_url = "/buckets/{}/collections/{}".format(
                bucket_id, collection_id
            )
            self.app.put_json(
                collection_url, MINIMALIST_COLLECTION, headers=self.headers
            )
            r = self.app.post_json(
...
class CollectionExpiresTest(BaseWebTest, unittest.TestCase):
    def setUp(self):
        ...
        resp = self.app.post_json(
            self.records_url, MINIMALIST_RECORD, headers=self.headers
        )
