...
class BucketViewTest(BaseWebTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        resp = self.app.put_json(
            self.record_url, MINIMALIST_BUCKET, headers=self.headers
        )
    ...
    def test_create_permissions_can_be_added_on_buckets(self):
        bucket = {
            **MINIMALIST_BUCKET,
            "permissions": {
                "collection:create": ["fxa:user"],
                "group:create": ["fxa:user"],
            },
        }
        resp = self.app.put_json(
            "/buckets/beers", bucket, headers=self.headers, status=200
        )
...
class BucketCreationTest(BaseWebTest, unittest.TestCase):
    ...
    def test_bucket_id_can_be_specified_in_post(self):
        ...
        r = self.app.post_json(
            "/buckets", {"data": {"id": bucket}}, headers=self.headers
        )
...
class BucketDeletionTest(BaseWebTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app.put_json(
            self.collection_url, MINIMALIST_COLLECTION, headers=self.headers
        )
    ...
    def test_buckets_can_be_deleted_in_bulk(self):
        self.app.put_json(
            "/buckets/1", MINIMALIST_BUCKET, headers=get_user_headers("alice")
        )
    ...
    def test_every_groups_are_deleted_too(self):
        ...
        # Verify tombstones
        resp = self.app.get(
            "{}/groups?_since=0".format(self.bucket_url), headers=self.headers
        )
        self.assertEqual(len(resp.json["data"]), 0)
    ...
    def test_can_be_created_after_deletion_with_if_none_match_star(self):
        ...
        self.app.put_json(
            self.bucket_url, MINIMALIST_BUCKET, headers=headers, status=201
        )
