...
class GroupViewTest(FormattedErrorMixin, BaseWebTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        resp = self.app.put_json(
            self.record_url, MINIMALIST_GROUP, headers=self.headers
        )
    ...
    def test_groups_can_have_arbitrary_attributes(self):
        ...
        resp = self.app.put_json(
            "/buckets/beers/groups/moderator", group, headers=self.headers
        )
        data = resp.json["data"]
    ...
    def test_groups_can_be_filtered_by_arbitrary_attribute(self):
        ...
        resp = self.app.get(
            "/buckets/beers/groups?has_size=true&min_size=2", headers=self.headers
        )
        data = resp.json["data"]
    ...
    def test_groups_should_reject_unaccepted_request_content_type(self):
        ...
        self.app.put(
            "/buckets/beers/groups/moderator",
            MINIMALIST_GROUP,
            headers=headers,
            status=415,
        )
    ...
    def test_recreate_group_after_deletion_returns_a_201(self):
        ...
        self.app.delete(
            "/buckets/beers/groups/moderator", headers=self.headers, status=200
        )
        self.app.put_json(
            "/buckets/beers/groups/moderator",
            MINIMALIST_GROUP,
    ...
    def test_group_doesnt_accept_groups_inside_groups(self):
        group = {
            **MINIMALIST_GROUP,
            "data": {"members": ["/buckets/beers/groups/administrators"]},
        }
        response = self.app.put_json(
            "/buckets/beers/groups/moderator", group, headers=self.headers, status=400
        )
...
class GroupManagementTest(BaseWebTest, unittest.TestCase):
    ...
    def test_group_is_removed_from_users_principals_on_group_deletion(self):
        self.app.put_json(
            self.group_url, MINIMALIST_GROUP, headers=self.headers, status=201
        )
        ...
        self.assertNotIn(
            self.group_url, self.permission.get_user_principals("fxa:user")
        )
    ...
    def test_group_is_added_to_user_principals_when_added_to_members(self):
        ...
        self.assertEquals(
            self.permission.get_user_principals("natim"),
            {"/buckets/beers/groups/moderators"},
        )
        self.assertEquals(
            self.permission.get_user_principals("mat"),
            {"/buckets/beers/groups/moderators"},
        )
    ...
    def test_group_is_added_to_user_principals_on_members_add_with_patch(self):
        ...
        self.app.get("/buckets/beers/groups", headers=self.headers, status=200)
        self.assertEquals(self.permission.get_user_principals("natim"), {group_url})
        self.assertEquals(self.permission.get_user_principals("mat"), {group_url})
        self.assertEquals(self.permission.get_user_principals("alice"), {group_url})
