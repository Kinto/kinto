...
class FunctionalTest(unittest.TestCase):
    ...
    def test_user_default_bucket_tutorial(self):
        ...
        resp = self.session.put(
            record_url, json={"data": task}, headers={"If-None-Match": "*"}
        )
        resp.raise_for_status()
        resp.raise_for_status()
        self.assertEquals(resp.status_code, 201)
    ...
    def test_user_shared_bucket_tutorial(self):
        ...
        self.assertIn(
            "system.Authenticated", collection["permissions"]["record:create"]
        )
        ...
        resp = self.session.post(
            collection_url, json={"data": alice_task}, auth=alice_auth
        )
