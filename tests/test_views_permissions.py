...
class PermissionsUnauthenticatedViewTest(BaseWebTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app.put_json(
            "/buckets/beers", MINIMALIST_BUCKET, headers=self.everyone_headers
        )
        self.app.put_json(
        ...
        # Other user.
        self.app.put_json(
            "/buckets/water", MINIMALIST_BUCKET, headers=get_user_headers("alice")
        )
    ...
    def test_permissions_can_be_paginated(self):
        ...
        real_permissions = self.app.get(
            "/permissions", headers=self.everyone_headers
        ).json["data"]
            ...
            for limit in range(1, 10):
                with self.subTest(order=order, limit=limit):
                    ...
                    while "Next-Page" in resp.headers:
                        next_url = resp.headers["Next-Page"][
                            len("http://localhost/v1") :
                        ]
                        resp = self.app.get(next_url, headers=self.everyone_headers)
                        records = records + resp.json["data"]

                    self.assertEqual(
                        sort_by_uri(real_permissions), sort_by_uri(records)
                    )
...
class EntriesTest(PermissionsViewTest):
    def setUp(self):
        ...
        self.app.put_json(
            "/buckets/beers/groups/amateurs", MINIMALIST_GROUP, headers=self.headers
        )
        self.app.put_json(
            "/buckets/beers/collections/barley/records/{}".format(RECORD_ID),
            MINIMALIST_RECORD,
    ...
    def test_permissions_can_be_paginated(self):
        ...
        resp = self.app.get(
            "/permissions?resource_name=record&_limit=7", headers=self.headers
        )
...
class GroupsPermissionTest(PermissionsViewTest):
    def setUp(self):
        ...
        self.admin_principal = self.app.get("/", headers=self.admin_headers).json[
            "user"
        ]["id"]
...
class DeletedObjectsTest(PermissionsViewTest):
    def setUp(self):
        ...
        self.app.put_json("/buckets/beers", MINIMALIST_BUCKET, headers=self.headers)
        self.app.put_json(
            "/buckets/beers/groups/admins", MINIMALIST_GROUP, headers=self.headers
        )
        self.app.put_json(
            "/buckets/beers/collections/barley",
            MINIMALIST_COLLECTION,
