...
def create_bucket(request, bucket_id):
    ...
    bucket_put = request.method.lower() == "put" and request.path.endswith(
        "buckets/default"
    )
    ...
    bucket = resource_create_object(
        request=request, resource_cls=Bucket, uri=bucket_uri
    )
...
def create_collection(request, bucket_id):
    ...
    collection_put = request.method.lower() == "put" and request.path.endswith(
        collection_id
    )
    if collection_put:
        return
...
def includeme(config):
    ...
    config.add_view(
        default_bucket, route_name="default_bucket", permission=NO_PERMISSION_REQUIRED
    )
    config.add_view(
        default_bucket,
