...
def allowed_from_settings(settings, principals):
    ...
    """
    perms_settings = {
        k: aslist(v) for k, v in settings.items() if k.endswith("_principals")
    }
    from_settings = {}
...
class PermissionsModel:
    ...
    def get_records(
        ...
        from_settings = allowed_from_settings(
            self.request.registry.settings, principals
        )
        ...
        if allowed_resources:
            ...
            for bucket in every_bucket:
                ...
                for res in allowed_resources:
                    ...
                    if res == "bucket":
                        perms_by_object_uri.setdefault(bucket_uri, set()).update(
                            resource_perms
                        )
                        continue
                    # Fetch bucket collections and groups.
                    # XXX: wrong approach: query in a loop!
                    every_subobjects, _ = storage.get_all(
                        parent_id=bucket_uri, collection_id=res
                    )
                    for subobject in every_subobjects:
                        subobj_uri = bucket_uri + "/{0}s/{1}".format(
                            res, subobject["id"]
                        )
                        perms_by_object_uri.setdefault(subobj_uri, set()).update(
                            resource_perms
                        )
...
class Permissions(resource.ShareableResource):
    ...
    def _extract_sorting(self, limit):
        ...
        without_last_modified = [
            s for s in result if s.field != self.model.modified_field
        ]
