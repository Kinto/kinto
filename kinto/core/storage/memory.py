...
class MemoryBasedStorage(StorageBase):
    ...
    def bump_timestamp(
        self, collection_timestamp, record, modified_field, last_modified
    ):
        ...
        """
        is_specified = (
            record is not None and modified_field in record or last_modified is not None
        )
        if is_specified:
...
class Storage(MemoryBasedStorage):
    ...
    def create(
        ...
        self.set_record_timestamp(
            collection_id, parent_id, record, modified_field=modified_field
        )
        _id = record[id_field]
    ...
    def get_all(
        ...
        deleted = []
        if include_deleted:
            deleted = _get_objects_by_parent_id(
                self._cemetery, parent_id, collection_id
            )
    ...
    def delete_all(
        ...
        records = _get_objects_by_parent_id(
            self._store, parent_id, collection_id, with_meta=True
        )
        records, count = self.extract_record_set(
            records=records,
            filters=filters,
...
def apply_sorting(records, sorting):
    ...
    for sort in reversed(sorting):
        result = sorted(
            result, key=lambda r: column(r, sort.field), reverse=(sort.direction < 0)
        )
    ...
    for pid, collections in by_parent_id.items():
        ...
        for collection, colobjects in collections.items():
            for r in colobjects.values():
                if with_meta:
                    objects.append(
                        dict(__collection_id__=collection, __parent_id__=pid, **r)
                    )
                else:
                    objects.append(r)
    return objects
