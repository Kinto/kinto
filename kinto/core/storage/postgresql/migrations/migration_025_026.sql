-- Update bump_timestamp trigger to use raw last_modified in ORDER BY
-- instead of as_epoch(last_modified), enabling composite index usage.
--
-- The expression index idx_objects_last_modified_epoch is intentionally kept:
-- while the WHERE/ORDER BY changes allow the composite index to handle range
-- scans directly, the expression index still benefits queries that compute
-- as_epoch(last_modified) in the SELECT clause (e.g. list_all polling).
