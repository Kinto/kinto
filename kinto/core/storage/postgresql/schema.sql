...
CREATE OR REPLACE FUNCTION as_epoch_micro(ts TIMESTAMP) RETURNS NUMERIC AS $$
BEGIN
...
-- Update the index to use as_epoch_micro for better precision
...
CREATE INDEX IF NOT EXISTS idx_objects_last_modified_epoch_micro
    ON objects(as_epoch_micro(last_modified));
...
BEGIN
    ...
    SELECT as_epoch_micro(MAX(last_modified)) INTO previous
    ...
    current := as_epoch_micro(clock_timestamp()::TIMESTAMP);
    IF previous IS NOT NULL AND previous >= current THEN
        current := previous + 1;
    END IF;

    IF NEW.last_modified IS NULL OR
       (previous IS NOT NULL AND as_epoch_micro(NEW.last_modified) = previous) THEN
