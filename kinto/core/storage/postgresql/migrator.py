...
class MigratorMixin:
    ...
    def create_or_migrate_schema(self, dry_run=False):
        ...
        logger.info(
            "Detected PostgreSQL {} schema version {}.".format(self.name, version)
        )
    ...
    def create_schema(self, dry_run):
        ...
        if not dry_run:
            ...
            logger.info(
                "Created PostgreSQL {} schema (version {}).".format(
    ...
    def migrate_schema(self, start_version, dry_run):
        ...
        for migration in migrations:
            ...
            logger.info(
                "Migrate PostgreSQL {} schema from version {} to {}.".format(
                    self.name, *migration
                )
            )
            ...
            logger.info(
                "Execute PostgreSQL {} migration from {}".format(self.name, filepath)
            )
            if not dry_run:
