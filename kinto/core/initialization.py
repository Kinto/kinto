...
def setup_version_redirection(config):
    ...
    def _redirect_to_version_view(request):
        ...
        querystring = request.url[
            (request.url.rindex(request.path) + len(request.path)) :
        ]
...
def _end_of_life_tween_factory(handler, registry):
    ...
    deprecation_msg = (
        "The service you are trying to connect no longer exists" " at this location."
    )
    ...
    if settings["statsd_url"]:
        ...
        if isinstance(policy, MultiAuthenticationPolicy):
            for name, subpolicy in policy.get_policies():
                client.watch_execution_time(
                    subpolicy, prefix="authentication", classname=name
                )
        else:
...
def install_middlewares(app, settings):
    ...
    if asbool(settings.get("profiler_enabled")):
        ...
        app = ProfilerMiddleware(
            app, profile_dir=profile_dir, restrictions=("*kinto.core*")
        )
...
def setup_logging(config):
    ...
    * http://12factor.net/logs
    ...
    def on_new_response(event):
        ...
        if summary_logger.level == logging.DEBUG:
            request.log_context(
                response=dict(headers=dict(response.headers), body=response.body)
            )
    ...
    for name in listeners:
        ...
        try:
            ...
            module_value = utils.read_env(
                "{}.{}".format(project_name, module_setting),
        ...
        actions_value = utils.read_env(
            "{}.{}".format(project_name, actions_setting),
        ...
        resource_value = utils.read_env(
            "{}.{}".format(project_name, resource_setting),
...
def initialize(config, version=None, project_name="", default_settings=None):
    ...
    project_name = (
        settings.pop("kinto.project_name", settings.get("project_name")) or project_name
    )
