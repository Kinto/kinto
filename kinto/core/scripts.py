    ...
    for backend in ("cache", "storage", "permission"):
        if hasattr(registry, backend):
            if readonly_mode and backend in readonly_backends:
                message = (
                    "Cannot migrate the {} backend while "
...
def delete_collection(env, bucket_id, collection_id):
    ...
    try:
        registry.storage.get(
            collection_id="collection", parent_id=bucket, object_id=collection_id
        )
    except storage_exceptions.RecordNotFoundError:
    ...
    registry.permission.delete_object_permissions(
        ...
        *[
            record.format(
                bucket_id=bucket_id, collection_id=collection_id, record_id=r["id"]
            )
            for r in deleted
        ]
    )
