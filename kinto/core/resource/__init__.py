...
class UserResource:
    ...
    def id_generator(self):
        ...
        id_generator = self.request.registry.id_generators.get(
            resource_name, default_id_generator
        )
        return id_generator
    ...
    def timestamp(self):
        ...
        try:
            ...
            raise http_error(
                HTTPServiceUnavailable(), errno=ERRORS.BACKEND, message=error_msg
            )
    ...
    def patch(self):
        ...
        body_behavior = self.request.validated["header"].get(
            "Response-Behavior", "full"
        )
        ...
        timestamp = new_record.get(
            self.model.modified_field, existing[self.model.modified_field]
        )
        self._add_timestamp_header(self.request.response, timestamp=timestamp)
    ...
    def _404_for_record(self, record_id):
        ...
        return http_error(
            HTTPNotFound(), errno=ERRORS.INVALID_RESOURCE_ID, details=details
        )
        ...
        for param, value in queryparams.items():
            ...
            if param in ("_since", "_to", "_before"):
                ...
                if param == "_since":
                    ...
                    if param == "_to":
                        message = (
                            "_to is now deprecated, " "you should use _before instead"
                        )
                        url = (
            ...
            if operator in (COMPARISON.IN, COMPARISON.EXCLUDE):
                ...
                has_invalid_value = (
                    field == self.model.id_field and not all_strings
                ) or (field == self.model.modified_field and not all_integers)
...
class ShareableResource(UserResource):
    ...
    def process_record(self, new, old=None):
        ...
        if self._is_json_patch:
            ...
            recursive_update_dict(
                permissions, payload.get("permissions", {}), ignores=(None,)
            )
