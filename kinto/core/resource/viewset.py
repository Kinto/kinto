...
class ViewSet:
    ...
    def get_view_arguments(self, endpoint_type, resource_cls, method):
        ...
        response_schemas = self.responses.get_and_bind(
            endpoint_type, method, record=record_schema
        )
    ...
    def get_name(self, resource_cls):
        ...
        has_class_attr = hasattr(resource_cls, "name") and not callable(
            resource_cls.name
        )
        if has_class_attr:
...
class ShareableViewSet(ViewSet):
    ...
    def get_record_schema(self, resource_cls, method):
        """Return the Cornice schema for the given method.
        """
        record_schema = super(ShareableViewSet, self).get_record_schema(
            resource_cls, method
        )
