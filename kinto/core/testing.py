...
skip_if_no_postgresql = unittest.skipIf(
    sqlalchemy is None, "postgresql is not installed."
)
skip_if_no_memcached = unittest.skipIf(memcache is None, "memcached is not installed.")
skip_if_no_statsd = unittest.skipIf(
    not statsd.statsd_module, "statsd is not installed."
)
...
class FormattedErrorMixin:
    ...
    def assertFormattedError(
        self, response, code, errno, error, message=None, info=None
    ):
