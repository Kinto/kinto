        ...
        if not allowed and is_list_operation:
            allowed = bool(
                context.fetch_shared_records(
                    permission, principals, self.get_bound_permissions
                )
            )
            if not allowed:
...
class RouteFactory:
    ...
    def __init__(self, request):
        ...
        if is_on_resource:
            ...
            if not self.on_collection and request.method.lower() in (
                "put",
                "delete",
                "patch",
            ):
    ...
    def fetch_shared_records(self, perm, principals, get_bound_permissions):
        ...
        by_obj_id = self._get_accessible_objects(
            principals, bound_perms, with_children=False
        )
        ids = by_obj_id.keys()
    ...
    def get_permission_object_id(self, request, object_id=None):
        ...
        if self.on_collection and object_id is not None:
            ...
            try:
                object_uri = utils.instance_uri(
                    request, self.resource_name, **matchdict
                )
